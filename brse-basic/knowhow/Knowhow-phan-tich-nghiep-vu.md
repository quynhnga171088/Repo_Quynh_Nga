...
---
**Nguồn:** `Kinh nghiệm test\Học về SQL và Database.docx`

Dùng SQL TryIt Editor để tập viết SQL query
Xem và lấy dữ liệu:
SELECT [tên trường] FROM [tên bảng]
...
SELECT * FROM [Customers]
WHERE ContactName LIKE ‘%ria%’
Sắp xếp dữ liệu với ORDER BY
VD: SELECT * FROM Customers
WHERE Price >30 
ORDER BY Productname ASC/DESC                              => sắp xếp tăng dần/giảm dần
ORDER BY Price ASC/DESC
Lấy dữ liệu từ nhiều bảng bằng lệnh JOIN
Cú pháp: FROM [Bảng] JOIN [Bảng] ON [Điều kiện]
Có thể join > 2 bảng
Ví dụ:
SELECT * 
FROM Products
JOIN Categories ON Products. CategoryID = Categories. CategoryID
Trường hợp lấy dữ liệu nằm trong cả hai bảng đều có thì dùng cú pháp: INNER  JOIN 
Trường hợp lấy toàn bộ dữ liệu nằm ở hai bảng thì dùng cú pháp: FULL JOIN
Trường hợp lấy dữ liệu nằm ở bảng bên trái thì dùng cú pháp: LEFT JOIN
Trường hợp lấy dữ liệu nằm ở bảng bên phải thì dùng cú pháp: RIGHT JOIN
Nếu bị trùng tên cột ở hai bảng thì cần chỉ rõ là muốn SELECT trường nào của bảng nào:
...
FROM [tên bảng] [alias]
Ví dụ:
SELECT 	p.CategoryID, p.ProductName, c.*
FROM Products p
LEFT JOIN Categories c ON p. CategoryID = c. CategoryID
Gom nhóm dữ liệu bẳng GROUP BY:
Cú pháp:
SELECT [trường], SUM/COUNT/AVG()
FROM [bảng]
GROUP BY [trường]
Ví dụ: 
...
---
**Nguồn:** `Kinh nghiệm test\Test_BasicKnowhow.xlsx`
...
Backend
HTML tĩnh
Check data import từ excel
## Sheet: UI
No | Chức năng | Description | Expected
1 | GUI general | Môi trường kiểm thử đã được “Clear”? | Môi trường kiểm thử đã sẵn sàng
...
31
32 | Kiểm tra thông tin header | - Image, slogan, favicon của công ty hiển thị đúng, rõ ràng, đồng nhất trên các màn hình khác nhau
-  Kích thước hợp lý, không quá to, không quá nhỏ, không bị vỡ trên các trình duyệt khác nhau
33 | Kiểm tra thông tin hiển thị trong mục footer | Hiển thị thông tin tên công ty, địa chỉ, số điện thoại liên lạc, đồng nhất trên các trang khác nhau
34 | Kiểm tra baner | Kiểm tra banner đúng thiết kế, đặt đúng vị trí
35 | Kiểm tra menu phải của Front End Screen | -  Phải có đủ các menu và sub-menu
36 | - Kế thừa trên các trang Front end khác
37 | Kiểm tra menu phải của Back End Screen | - Có menu và sub-menu đầy đủ
38 | - Kế thừa trên các trang Back end khác
39 | Chuyển trạng thái ngôn ngữ | Kiểm tra chính tả, kiểm tra có thể chuyển đổi giữa hai ngôn ngữ tiếng Anh và tiếng Nhật
## Sheet: Search
No | Chức năng | Description
1 | Search | 1.1. Validate giao diện:
2 | - Không cần check validate các trường text box như maxlength, ...
3 | - Chỉ cần validate các trường ngày tháng, combobox
4 | - Luôn phải nhớ các case kiểm tra danh sách bao gồm tiêu đề các cột, căn lề, các tooltip và phân trang
5 | 1.2. Chức năng:
6 | - Tìm kiếm theo giá trị mặc định (không nhập tiêu chí tìm kiếm nào)
7 | - Tìm kiếm Không ra kết quả phải có màn hình hiển thị thông báo thích hợp
8 | - Tìm kiếm theo từng tiêu chí đơn lẻ
9 | - Tìm kiếm kết hợp các tiêu chí
10 | - Kiểm tra trong kết quả tìm kiếm Không bị trùng lặp kết quả
11 | - Người dùng có thể bắt đầu tìm kiếm trên bàn phím keyboard
12 | - Màn hình hiển thị thông báo cho bất kỳ trường hợp nào bị delay trong quá trình tìm kiếm
13 | 1.3. Testcase tìm kiếm đơn lẻ
14 | a. Đối với text box:
15 | - Tìm kiếm khi nhập toàn ký tự khoảng trắng
16 | - Tìm kiếm khi nhập chuỗi có chứa ký tự khoảng trắng đầu cuối
17 | - Tìm kiếm khi nhập chuỗi có ký tự hoa thường
18 | - Tìm kiếm khi nhập tiếng việt có dấu
19 | - Tìm kiếm khi nhập các ký tự đặc biệt
20 | b. Đối cới combobox:
21 | - Tìm kiếm khi Combobox = ----Chọn---- Hoặc Tât cả
22 | - Tìm kiếm khi chọn giá trị bất kỳ trong combobox
23 | c. Đối với checkbox:
24 | - Tìm kiếm khi check chọn 1 giá trị của Checkbox
25 | - Tìm kiếm khi check chọn nhiều giá trị trong list của checkbox
26 | d. Đối với radio button
27 | - Tìm kiếm khi tích chọn 1 radio button
28 | e. Đối với ngày tháng: Từ ngày
29 | - Tìm kiếm khi nhập từ ngày là ngày hiện tại
30 | - Tìm kiếm khi nhập từ ngày là 1 ngày bất kỳ
31 | f. Đối với ngày tháng: Đến ngày
32 | - Tìm kiếm khi nhập đến ngày là ngày hiện tại
33 | - Tìm kiếm khi nhập đến ngày là 1 ngày bất kỳ
34 | g. Đối với tìm kế hợp từ ngày – đến ngày
35 | - Tìm kiếm khi nhập từ ngày = đến ngày
36 | - Tìm kiếm khi nhập từ ngày , đến ngày
37 | 1.4. Testcase tìm kiếm kết hợp
38 | - Tìm kiếm theo tất cả tiêu chí tìm kiếm
39 | - Tìm kiếm theo 1 vài tiêu chí tìm kiếm
40 | 1.5 . Đối với hiển thị phân trang (ex: giả sử phân trang là 20 records - một trang chỉ chứa được 20 dòng)
41 | - Search được 0 record thì chỉ hiển thị header (phần datagrid chỉ hiển thị header)
42 | - Search được 20 records thì hiển thị header và 20 dòng này trên 1 trang
43 | - Search được 21 records thì hiển thị header và 20 dòng này trên trang 1 và 1 dòng trên trang 2.
44 | -  Search được 50 records thì hiển thị header và 20 dòng này trên 1 trang và 20 dòng trên trang 2 và 10 dòng trên trang 3
45 | - Khả năng lưu giữ điều kiện Tìm kiếm khi thực hiện phân trang
46 | - Trong trường hợp đang hiển thị nhiều trang thì click Next và Prev để kiểm tra xem có qua lại giữa các trang
47 | - Kiểm tra khi đang ở trang cuối cùng thì link chuyển đến last page bị disable
48 | - Kiểm tra khi đang ở trang đầu tiên thì link chuyển đến first page bị disable
49 | -  Kiểm tra sự kiện next , previous trang khi có nhiều trang. Di chuyển đến trang tiếp theo trang trước trang đó
50 | - Kiểm tra khi nhấn vào một trang bất kì. Di chuyển đến trang tương ứng
## Sheet: Add new
Chức năng | Description
Add new | Kiểm tra hiển thị confirm trước khi thêm
...
Kiểm tra tất cả các trường bắt buộc phải được validate và phải được chỉ định bằng dấu hoa thị *
Kiểm tra khi nhấn nút thêm mới liên tục
## Sheet: Edit
Chức năng | Description
Edit | Validate giao diện
...
## Sheet: Deleted
Chức năng | Description
Deleted | Kiểm tra hiển thị thông báo khi xóa
...
Kiểm tra mở lại content đã xóa thành công bằng url
## Sheet: Link
Chức năng | Description
Link | Link từ trang này đến trang khác phải hoạt động
Link từ trang này đến trang khác phải đúng trang
## Sheet: Textbox
Chức năng | Description
Textbox | Kiểm tra các giá trị biên của textbox
Kiểm tra các giá trị hợp lệ
Kiểm tra khi nhập các kí tự HTML, Javasript (vd: <html>abc</html> hoặc <script> alert ('Hello') </script>).
Kiểm tra khi nhập các kí tự đặc biệt (kí tự @, ~,!,...)
Kiểm tra khi nhập giá trị SPACE đầu, cuối/ hoặc để trống (với textbox là những trường bắt buộc thì cần check xem có chấp nhận kí tự trống hay không)
Kiểm tra khi nhập giá trị emoji
Kiểm tra khi thực hiện CTRL+V; CTRL+C; CTRL+X, cách kiểm tra  này nên được kiểm thử hai chiều, đặc biệt là với textbox dành cho mật khẩu.
Kiểm tra khi ấn nút TAB thì con trỏ có di chuyển giữa các textbox theo thứ tự từ trái sang phải, từ trên xuống dưới (đối với nhiều textbox)
Kiểm tra khi ấn SHIFT+TAB thì con trỏ có di chuyển giữa các textbox theo thứ tự ngược lại, từ phải sang trái, từ dưới lên trên(đối với nhiều textbox)
Các textbox Readonly: Tất cả các trường và điều khiển đều không có hiệu lực trong chế độ Read_only không?
Text area: Kiểm tra các giá trị biên của field
Kiểm tra dấu con trỏ được hiển thị trên textbox hay không
Kiểm tra dữ liệu sau khi nhập vào có được canh lề hay không
...
Một số loại textbox với định dạng như sau(Tuy nhiên cũng còn tùy thuộc vào yêu cầu của mỗi dự án):
...
Textbox cho trường Email
1. Valid Email address
Kiểm tra địa chỉ email hợp lệ: email@domain.com
Kiểm tra email chứa dấu chấm : first.lastname@domain.com
Kiểm tra địa chỉ email có tên miền phụ: email@subdomain.domain.com
Kiểm tra tên miền là địa chỉ IP: email@123.123.123.123 và trong khung vuông: email@[123.123.123.123]
Kiểm tra địa chỉ email có chứa dấu nháy đơn : “email”@domain.com
Kiểm tra địa chỉ email có chứa chữ số :  1234567890@domain.com
Kiểm tra địa chỉ email có chứa dấu gạch dưới/gạch ngang : ______@domain.com / firstname-lastname@domain.com
Kiểm tra địa chỉ email với  'tên miền hợp lệ:  email@domain.name / email@domain.co.jp
2. Invalid Email address
Kiểm tra địa chỉ email không có giá trị ‘@’ và tên miền
Kiểm tra địa chỉ email có giá trị đặc biệt: #@%^%#$@#$@#.com
Kiểm tra địa chỉ email không có tên miền : @domain.com
Kiểm tra thẻ html trong email không hơp lê: le quyen <email@domain.com>
Kiểm tra địa chỉ email thiểu giá trị’@’: email.com
Kiểm tra dịa chỉ emal chứa 2 giá trị ‘@’ : email@domain@domain.com
Kiểm tra dấu chấm bắt đầu địa chỉ email :  .email@domain.com
Kiểm tra địa chỉ email chứa nhiều dấu chấm:  ...email@domain.com
Kiểm tra 'Unicode trong địa chỉ email: あいうえお@domain.com/ LệQuyện@domain.com
Kiểm tra sau tên miền của email chứ văn bản:  email@domain.com (Quyên)
Kiểm tra thiếu top level domain (.com/.net/.org/etc) : email@domain
Kiểm tra tên miền của email có chứa dấu gạch ngang: email@-domain.com
Kiểm tra format địa chỉ IP không hợp lê: email@111.222.333.44444
Kiểm tra có nhiều dấu chấm trong tên miền: email@domain..com
Textbox cho trường user name (ID), password trên màn hình login
Kiểm tra chức năng  không nhập trường nào
...
Kiểm tra trường hợp nhập [Username] có ít hơn 4 kí tự
Kiểm tra trường hợp nhập [Username] có 4 kí tự
...
Kiểm tra trường hợp nhập đúng tài khoản đã đăng ký nhưng chưa được kích hoạt
Kiểm tra trường hợp nhập đúng tài khoản đã đăng ký nhưng bị block
...
## Sheet: Selection box
Chức năng | Description
Selection box | Kiểm tra chức năng Sort của các cột trong phần danh sách
Kiểm tra các nội dung trong selection box đúng và đủ
Kiểm tra dùng chuột để chọn 1 đối tượng trong danh sách
Kiểm tra nhập tìm kiếm theo chữ cái đầu tiền của danh sách các đối tượng
Kiểm tra danh sách các đối tượng được sắp xếp theo bảng chữ cái
Kiểm tra việc hiển thị scroll bar ngang và dọc để có thể xem hết được nội dung
## Sheet: Dropdownn list
Chức năng | Description
Dropdownn list | Kiểm tra các giá trị trong combobox được sắp xếp theo thứ tự alphabet.
Kiểm tra việc sử dụng phím lên, xuống và phím enter để lựa chọn các giá trị.
Kiểm tra việc  nhập tìm kiếm theo chữ cái đầu tiền của danh sách các giá trị
Kiểm tra nội dung giá trị nó dài => thêm scroll
...
## Sheet: Radio button
Chức năng | Description
Radio button | Kiểm tra việc set giá trị mặc định
Kiểm tra việc chọn nhiều giá trị cùng 1 lúc
Kiểm tra kích thước, màu sắc,
Kiểm tra xem điều khiển nút radio có thể được chọn hoặc bỏ chọn bằng phím Tab hay  với sự trợ giúp của con trỏ chuột
## Sheet: Checkbox
Chức năng | Description
Checkbox | Kiểm tra nếu Click 1 lần và 2 lần hộp checkbox
Kiểm tra có thể không chọn checkbox hoặc chọn một giá trị
Kiểm tra chọn nhiều hộp checkbox
## Sheet: Button
Chức năng | Description
Button | Kiểm tra xử lý khi click vào button này (vd: chuyển đến trang khác, thêm, cập nhật, xoá...)
Kiểm tra kích thước, background button
Kiểm tra font - màu - kích cỡ text
Kiểm tra việc nhấn nhiều lần vào button
## Sheet: DateTime
Chức năng | Description
DateTime | Kiểm tra ngày giờ của chức năng lấy theo ngày giờ server hệ thống hay client
...
Kiểm tra có thể chọn ngày mong muốn trên lịch.
Kiểm tra xem trường lịch có thể chỉnh sửa được ngay cả khi người dùng có thể chọn ngày biểu mẫu lịch
Kiểm tra nhập blank space cho (ngày/tháng/năm)
Kiểm tra  nhập ngày không hợp lệ và tháng và năm hợp lệ
Kiểm tra nhập ngày và tháng hợp lệ và năm không hợp lệ
Kiểm tra  nhập ngày hợp lệ và tháng không hợp lệ và năm hợp lệ
Kiểm tra  nhập ngày hợp lệ và tháng không hợp lệ và năm không hợp lệ
Kiểm tra  nhập ngày không hợp lệ và tháng không hợp lệ và năm không hợp lệ
Kiểm tra  nhập ngày không hợp lệ và tháng hợp lệ và năm không hợp lệ
Kiểm tra  nhập khoảng trống không hợp lệ và tháng hợp lệ không hợp lệ
Kiểm tra bảng  lịch có mở trong tất cả các trình duyệt không
## Sheet: Table
Chức năng | Description
Table | Kiểm tra hiển thị table khi không có bản ghi hoặc khi đã có bản ghi rồi
Kiểm tra chức năng sort của table
## Sheet: Scroll bar
Chức năng | Description
Scroll bar | Kiểm tra hướng của scroll bar (ngang, dọc)
## Sheet: Pop-up message dialog, alert
Chức năng | Description
Pop-up message dialog, alert | Kiểm tra pop-up msg được hiển thị ở phía client hay server.
Kiểm tra thứ tự thông báo lỗi của các hộp thoại trên trang khi không kết nối được tới DB phải có thông báo cụ thể chứ không được hiển thị ra trang lỗi của trình duyệt. Phải có thông báo lỗi khi xảy ra lỗi nhập liệu trên màn hình.
Kiểm tra thông báo trên tất cả các trình duyệt
Kiểm tra chính tả và ngữ pháp của nội dung tin nhắn
Kiểm tra tin nhắn hiển thị theo trình duyệt khác nhau
Kiểm tra với những thông báo lỗi phải được hiển thị màu đỏ .
Kiểm tra tất cả các thông báo cảnh báo sẽ hiển thị dấu hỏi
Kiểm tra phông chữ, căn chỉnh, kích thước và màu sắc của tin nhắn
Kiểm tra thời gian tin nhắn hiển thị.
Kiểm tra những tin nhắn chỉ hiển thị 1 lần xem có hiển thị trên các trang các lần thứ 2 nữa không
## Sheet: Zoom in & Zoom out
Chức năng | Description
Zoom in | Khi nhấn vào biểu tượng Zoom in . Giao diện hiển thị nhỏ lại tùy theo độ zoom mà không bị lệch giữa các phần.
Khi nhấn Ctrl+ Chuột theo chiều hướng vào trong. Giao diện hiển thị nhỏ lại tùy theo độ zoom và không bị lệch giữa các phần.
Zoom out | Khi nhấn vào biểu tượng Zoom out giao diện hiển thị to lên tùy theo độ zoom mà không bị lệch giữa các phần.
Khi nhấn Ctrl+ Chuột theo chiều hướng ra ngoài giao diện hiển thị to ra tùy theo độ zoom và không bị lệch giữa các phần.
## Sheet: Image file upload
Chức năng | Description
Image/file upload | Kiểm tra đường dẫn hình ảnh được tải lên
Kiểm tra chức năng tải lên hình ảnh và thay đổi
Kiểm tra chức năng tải lên hình ảnh với các tệp hình ảnh của các tiện ích mở rộng khác nhau (ví dụ: JPEG, PNG, BMP, v.v.)
Kiểm tra chức năng tải lên hình ảnh với  tên chứa kí tự rỗng hoặc một số ký tự đặc biệt.
Kiểm tra tải lên hình ảnh tên trùng lặp
Kiểm tra tải lên hình ảnh với kích thước hình ảnh lớn hơn kích thước tối đa được phép. Thông báo lỗi thích hợp sẽ được hiển thị.
Kiểm tra chức năng tải lên hình ảnh với các loại tệp khác với hình ảnh (ví dụ: txt, doc, pdf, exe, v.v.). Kiểm tra xác nhận cho định dạng không hợp lệ
Kiểm tra xem hình ảnh có chiều cao và chiều rộng được chỉ định (với spec cụ thể) có được chấp nhận hay không
Kiểm tra  thanh bar hiển thị với hình ảnh kích thước lớn.
Kiểm tra xem chức năng nút "cancel" trong khi upload hình tải lên .
Kiểm tra  hộp thoại chọn tệp chỉ hiển thị các tệp được hỗ trợ được liệt kê
Kiểm tra nhiều chức năng tải lên hình ảnh
Kiểm tra chất lượng hình ảnh sau khi upload. Chất lượng hình ảnh không được thay đổi sau khi tải lên
Kiểm tra xem người dùng có thể sử dụng / xem hình ảnh được tải lên không
Kiểm tra đang trong quá trình upload thì ngắt kết nối mạng LAN
Kiểm tra chấp nhận nội dung tên file gồm chữ và số
Kiểm tra không tải lên bất kỳ hình ảnh và click vào nút tải lên.
Kiểm tra tải lên hình ảnh với một cái tên dài
Kiểm tra lấy tập tin văn bản và thay đổi phần mở rộng của nó sang định dạng hình ảnh được hỗ trợ (ví dụ: jpeg) và  tải lên.
## Sheet: Backend
Chức năng | Description
Check File CSV format | Kiểm tra với file CSV đúng định dạng
Kiểm tra với file rỗng
Kiểm tra với file CSV sai định dạng (.docx, .xmls, .zip, …)
Kiểm tra dung lượng file CSV (100 record. 500 record, …)
Kiểm tra file CSV quá dung lượng cho phép (VD: >1M)
Kiểm tra định dạng file CSV định dạng Encode Shift-JIS
Kiểm tra kí tự fullsize halfsize
Check Data File CSV | Kiểm tra các trường với valid data
Kiểm tra các trường với invalid data (VD: rỗng, quá kí tự,…)
Kiểm tra header name ( VD: rỗng, invalid,…)
Kiểm tra với required column
Kiểm tra với TH có cả chữ, số, kí tự đặc biệt
Kiểm tra với các kí tự đặc biệt (VD: !@#$%^&*…)
Kiểm tra với TH data giống nhau (TH duplicate)
Kiểm tra với TH data khác nhau
Kiểm tra với chữ Nhật (VD: Kanji, katakana, hiragana)
Kiểm tra insert thêm những dòng blank
Check import data | Kiểm tra việc import data có mất nhiều thời gian (check với các TH dung lượng file)
Kiểm tra MSG, icon hiển thị khi import
Kiểm tra việc import/ update/ delete
Check connection | Kiểm tra việc xử lý khi mất kêt nối (VD: hiển thị MSG error)
Check phạm vi ảnh hưởng | Kiểm tra với các chức năng đã phát triển từ trước
## Sheet: HTML tĩnh
Chức năng | Description
Check HTML tĩnh | Kiểm tra việc page có gọi API, JS ( Check trên F12/ Network)
Kiểm tra page sau khi chuyển thì giao diện có bị thay đổi không? ( Confirm thông qua requirement)
Kiểm tra cách đăt tên task về phía Backend
Kiểm tra folder import file/ source code (Accset) trên BackEnd (có đặt đúng folder KH mong muốn)
## Sheet: Check data import từ excel
Chức năng | Description
Import data from excel | Kiểm tra data import vào có đủ theo như file KH gửi
Kiểm tra thứ tự của data, số lượng data có trong 1 page
Kiêm tra text, font size ( TH dev copy thiếu/sai data)
## Sheet: Security
Chức năng | Description
Security
